...
которые помогут вам справиться с задачей. Главное, это реализовать метод to_dict(),
он является точкой входа для взаимодействия с вашим миксином и он должен вернуть
представление вашего объекта в виде словаря. Обратите внимание на вложенность атрибутов.
"""
...
class DictMixin:
    def to_dict(self):
        # Создаем словарь, который будет представлять объект
        result = {}

        # Получаем все атрибуты экземпляра
        for key, value in self.__dict__.items():
            # Если значение является экземпляром DictMixin, вызываем его to_dict()
