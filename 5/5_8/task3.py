"""
В Инди-курсе мы проходили  json и сериализацию, узнали, как конвертировать встроенные объекты python в json-строке.
А что произойдет, если мы попробуем сериализовать ЭК, который мы с вами написали.
Получим ошибку - TypeError: Object of type Person is not JSON serializable
...
Метод to_json() конвертирует словарь атрибутов экземпляра в строку JSON, используя стандартную библиотеку json в Python.
"""
import json
...
book.to_json() == ('{"title": "The Catcher in the Rye", '
                   '"author": "J.D. Salinger", '
                   '"ratings": [5, 4, 5, 4, 5], '
                   '"is_bestseller": true}')
